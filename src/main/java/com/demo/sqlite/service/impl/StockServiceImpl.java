...
public class StockServiceImpl implements StockService {
   ...
   public StockResponseDTO addProduct(String description, int categoryId, String status,
         double price, int quantity, int clientId, Optional<byte[]> imageOpt)
         throws ValidationError {
      Optional<Category> optionalCategory = categoryRepository.findById(categoryId);
      if (optionalCategory.isEmpty()) {
         throw new ValidationError(String.format("Category %d not found", categoryId));
      }
      Stock newStock = Stock.builder().description(description).categoryId(categoryId).status(status)
            .price(price).quantity(quantity).createdBy(clientId).updatedBy(clientId).build();
   ...
   @Transactional
   public Optional<StockResponseDTO> updateProduct(int code, String description, int categoryId,
         String status, double price, int quantity, int clientId, Optional<byte[]> imageOpt)
         throws ValidationError {
      Optional<Category> optionalCategory = categoryRepository.findById(categoryId);
      if (optionalCategory.isEmpty()) {
         throw new ValidationError(String.format("Category %d not found", categoryId));
      }
      return stockRepository.findById(code).map(stock -> {
