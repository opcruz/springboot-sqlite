...
public class ShoppingCartController {
   ...
   public @ResponseBody ResponseEntity<ShoppingCartResultResponseDTO> getShoppingCart(
         Authentication auth) {
   ...
   public @ResponseBody ResponseEntity<Order> buyCart(
         @Parameter(name = "payment_method", description = "Payment Method", in = ParameterIn.QUERY, schema = @Schema(type = "string", allowableValues = {
               "CASH", "VISA", "MASTERCARD",
               "PAYPAL" }), example = "CASH") @RequestParam(name = "payment_method") String paymentMethod,
         Authentication auth) {
      int clientId = UserAuthenticateInfo.fromAuth(auth).getUserId();
