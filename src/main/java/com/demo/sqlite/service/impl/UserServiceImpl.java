...
public class UserServiceImpl implements UserService {
   ...
   public Optional<LoginResponseDTO> login(LoginRequestDTO requestDTO, String role)
      ...
      if (Roles.CLIENT.getRole().equalsIgnoreCase(role)) {
         loginResponseDTOOptional = clientRepository
               .findClientByEmailAndPassword(requestDTO.getEmail(), passwordHash).map(client -> {
                  String token = getJWTToken(requestDTO.getEmail(),
                        Roles.CLIENT.getRoleWithPrefix(), client.getId());
                  return LoginResponseDTO.builder().email(requestDTO.getEmail()).token(token)
                        .build();
               });
      } else if (Roles.EMPLOYEE.getRole().equalsIgnoreCase(role)) {
         loginResponseDTOOptional = employeeRepository
               .findEmployeeByEmailAndPassword(requestDTO.getEmail(), passwordHash).map(client -> {
                  String token = getJWTToken(requestDTO.getEmail(),
                        Roles.EMPLOYEE.getRoleWithPrefix(), client.getId());
                  return LoginResponseDTO.builder().email(requestDTO.getEmail()).token(token)
                        .build();
               });
      } else {
   ...
   private String getJWTToken(String email, String role, int id) {
      return JWTCoder.generateJWT(email, id, Collections.singletonList(role));
   }
